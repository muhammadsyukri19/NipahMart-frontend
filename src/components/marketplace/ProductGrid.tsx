...
export function ProductGrid({ products }: { products: Product[] }) {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-8">
      {products.map((prod) => (
        <Link href={`/marketplace/product/${prod.slug}`} key={prod.id} className="group cursor-pointer block">
          {/* Product Image Box */}
          <div className="relative aspect-[4/5] w-full overflow-hidden rounded-sm mb-4 bg-white border border-surface-container-high">
            <img src={prod.images?.[0] || '/images/products/placeholder.webp'} alt={prod.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
          ...
          <div className="mt-auto">
            ...
            <h3 className="font-serif text-[20px] text-primary-dark font-bold leading-tight mb-2 group-hover:text-primary">{prod.name}</h3>
            ...
            <div className="flex justify-between items-center mt-3">
              ...
              {prod.rating > 0 && (
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-[12px] text-outline">({prod.reviewCount})</span>
                </div>
              )}
            </div>
          </div>
